...
async def create_feed(feed: FeedCreate, session: AsyncSession = Depends(get_session)):
    ...
    # 1. Write to YAML
    try:
        await manager.add_to_yaml(feed.name, feed.url, feed.enabled)
    except ValueError:
    ...
    try:
        ...
        await manager.remove_from_yaml(feed.url)
        raise HTTPException(status_code=409, detail="Feed with this URL already exists")
...
async def update_feed(feed_id: int, feed_update: FeedCreate, session: AsyncSession = Depends(get_session)):
    ...
    try:
        if old_url != feed_update.url:
            await manager.remove_from_yaml(old_url)
            await manager.add_to_yaml(feed_update.name, feed_update.url, feed_update.enabled)
        else:
            await manager.update_in_yaml(feed_update.url, feed_update.name, feed_update.enabled)
    except OSError as e:
        raise HTTPException(status_code=500, detail=f"Cannot write to sources.yaml: {e}")
